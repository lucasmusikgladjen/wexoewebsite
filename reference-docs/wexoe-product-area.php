...
if (!defined('WEXOE_PA_AIRTABLE_API_KEY')) define('WEXOE_PA_AIRTABLE_API_KEY', 'API_KEY_PLACEHOLDER');
if (!defined('WEXOE_PA_AIRTABLE_BASE_ID')) define('WEXOE_PA_AIRTABLE_BASE_ID', 'appXoUcK68dQwASjF');
...
if (!defined('WEXOE_PA_CACHE_TTL')) define('WEXOE_PA_CACHE_TTL', 300); // 5 minutes
...
function wexoe_pa_md($text) {
    if (empty($text)) return '';
    $t = esc_html($text);
    // Links: [text](url) — must be done before other inline formatting
...
function wexoe_pa_airtable_request($table, $params = []) {
    ...
    $response = wp_remote_get($url, [
        'headers' => [
            'Authorization' => 'Bearer ' . WEXOE_PA_AIRTABLE_API_KEY,
        ],
        'timeout' => 15,
    ]);
...
/**
 * Fetch product area by slug (cached)
 */
function wexoe_pa_fetch_product_area($slug) {
    $cache_key = 'wexoe_pa_area_' . sanitize_key($slug);
    $cached = get_transient($cache_key);
    if ($cached !== false) return $cached;

    $result = wexoe_pa_airtable_request(WEXOE_PA_TABLE_PRODUCT_AREAS, [
        'filterByFormula' => '{Slug}="' . addslashes($slug) . '"',
        'maxRecords' => 1,
    ]);
    ...
    set_transient($cache_key, $data, WEXOE_PA_CACHE_TTL);
    return $data;
}

/**
 * Fetch linked records by record IDs from a table (cached)
 */
function wexoe_pa_fetch_linked_records($table, $record_ids, $cache_prefix) {
    if (empty($record_ids)) return [];

    $cache_key = 'wexoe_pa_' . $cache_prefix . '_' . md5(implode(',', $record_ids));
    $cached = get_transient($cache_key);
    if ($cached !== false) return $cached;

    // Build OR formula for all record IDs
    $parts = [];
    ...
    do {
        ...
        $result = wexoe_pa_airtable_request($table, $params);
        ...
        $offset = isset($result['offset']) ? $result['offset'] : null;
    } while ($offset);
    ...
    // Filter by Visa field
    $all_records = array_filter($all_records, function($r) {
        return !empty($r['Visa']);
    });

    set_transient($cache_key, array_values($all_records), WEXOE_PA_CACHE_TTL);
    return array_values($all_records);
}

/**
 * Extract record IDs from an Airtable linked field
 */
...
/**
 * Fetch all articles linked to products via each product's "Articles" linked field
 * Returns associative array: product_record_id => [articles]
 */
function wexoe_pa_fetch_articles_for_products($products) {
    if (empty($products)) return [];

    // Gather all unique article record IDs from products' "Articles" field
    $all_article_ids = [];
    $product_article_map = []; // product_record_id => [article_ids]
    ...
    // Fetch articles from Airtable (no Visa filter — Articles table doesn't have it)
    $cache_key = 'wexoe_pa_articles_' . md5(implode(',', $all_article_ids));
    $cached = get_transient($cache_key);
    $articles_list = [];

    if ($cached !== false) {
        $articles_list = $cached;
    } else {
        $parts = [];
        foreach ($all_article_ids as $aid) {
            $parts[] = 'RECORD_ID()="' . $aid . '"';
        }
        $formula = 'OR(' . implode(',', $parts) . ')';
        $offset = null;
        ...
        do {
            ...
            foreach ($result['records'] as $rec) {
                $fields = $rec['fields'];
                $fields['_record_id'] = $rec['id'];
                $articles_list[] = $fields;
            }

            $offset = isset($result['offset']) ? $result['offset'] : null;
        } while ($offset);

        set_transient($cache_key, $articles_list, WEXOE_PA_CACHE_TTL);
    }

    // Index articles by record ID
    $articles_by_id = [];
...
function wexoe_product_area_shortcode($atts) {
    ...
    // Clear all plugin caches if nocache is set
    if ($atts['nocache'] === 'true') {
        global $wpdb;
        $wpdb->query("DELETE FROM {$wpdb->options} WHERE option_name LIKE '_transient_wexoe_pa_%' OR option_name LIKE '_transient_timeout_wexoe_pa_%'");
    }
...
function wexoe_pa_customer_lookup() {
    ...
    $result = wexoe_pa_airtable_request(WEXOE_PA_TABLE_CUSTOMERS, [
        'filterByFormula' => '{Customer ID}="' . addslashes($customer_id) . '"',
        'maxRecords' => 1,
    ]);
