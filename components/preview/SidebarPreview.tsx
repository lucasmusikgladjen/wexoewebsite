...
function CaseSidebar({ state, mainColor, secondaryColor }: { state: PageState; mainColor: string; secondaryColor: string }) {
  ...
  return (
    <div className="rounded-lg overflow-hidden border border-gray-200 bg-white shadow-sm">
      <div className="px-4 py-2 text-white text-sm font-semibold leading-tight" style={{ background: mainColor }}>
        {state.caseTitle || 'Kundcase'}
      </div>
      {state.caseImage && (
        <img src={state.caseImage} alt="" className="w-full h-32 object-cover" />
      )}
...
function EventSidebar({ state, mainColor, secondaryColor }: { state: PageState; mainColor: string; secondaryColor: string }) {
  return (
    <div className="rounded-lg overflow-hidden border border-gray-200 bg-white shadow-sm">
      ...
      <div className="px-4 py-3 space-y-2">
        <h4 className="font-semibold text-sm leading-tight" style={{ color: mainColor }}>{state.eventTitle || 'Event-titel'}</h4>
