/**
 * Product Page — Lager 3 create/update-overrides.
 *
 ...
 *
 * Här lever den tidigare app/api/product-page/route.ts-create/update-koden,
...
import { NormalSection, ProductPageState } from '../product-page-types';
import { buildProductPageTransform } from '../deterministic-transform';
...
function getSlot(state: ProductPageState, slot: SectionSlot): NormalSection {
...
async function loadExistingPa(
  ...
  const record = await getRecord(apiKey, PA_TABLE_IDS.productPages, recordId, PA_BASE_ID);
  return {
...
export async function productPageCreate(
  ...
  const transformed = buildProductPageTransform(workingState, 'create');
  ...
  // CREATE Product Page:n med rätt link-array-ordning. `Name` mirroras
  ...
  const createdPa = await createRecord(airtableKey, PA_TABLE_IDS.productPages, paFields, PA_BASE_ID);
...
export async function productPageUpdate(
  recordId: string,
  state: ProductPageState,
  ctx: { apiKey: string },
  ...
  const transformed = buildProductPageTransform(state, 'update');
  ...
  // PATCH Product Page:n med transformens fält + nya link-array-ordningar.
  ...
  await updateRecord(airtableKey, PA_TABLE_IDS.productPages, recordId, {
    ...transformed.productPage,
    product_ids: productIdOrder,
    solution_ids: solutionIdOrder,
...
export async function listProductPages(apiKey: string): Promise<ProductPageListItem[]> {
  const records = await listRecords(apiKey, PA_TABLE_IDS.productPages, {
    baseId: PA_BASE_ID,
