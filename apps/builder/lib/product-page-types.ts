...
//
// Mirrors the Airtable "cms_product_pages" table plus enough of the linked
...
/** Initial state for a brand-new Product Page. All colour fields start empty
 *  so the PHP plugin's built-in defaults (navy / orange / alternating grey)
 *  render through `colorOr()` in the preview. */
export function emptyProductPageState(): ProductPageState {
  return {
    mode: 'create',
    recordId: '',
    slug: '',
