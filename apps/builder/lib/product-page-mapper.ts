/**
 * Reverse mapping from Airtable records → ProductPageState, used by the
 * loader to hydrate the builder when editing an existing Product Page.
 *
 * Forward mapping (state → Airtable fields) lives in the deterministic write
 * path (`buildProductPageTransform` in `lib/deterministic-transform.ts`); this
...
export const PA_TABLE_IDS = {
  productPages: 'tbl5PQR7FNHCogeya',       // cms_product_pages
...
export function productPageStateFromRecords(args: {
  productPage: AirtableRecord;
  products: AirtableRecord[];
  articles: AirtableRecord[];
  solutions: AirtableRecord[];
  sections: AirtableRecord[];
}): ProductPageState {
  const { productPage, products, articles, solutions, sections } = args;
  const f = productPage.fields;
  ...
  return {
    ...
    // sektionen har innehåll, annars "av". Speglar legacy ProductPageBuilder:s
    // visibility-state.
    ...
    contactForm: contactFormFromFields(productPage.fields, 'snake_case'),
  };
}
